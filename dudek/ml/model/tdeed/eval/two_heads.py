...
_NOT_APPLICABLE_TEAM = Team.NOT_APPLICABLE
...
class _TeamBASScoredVideo(TeamBASScoredVideo):
    ...
    def from_predictions(
        ...
        targets = video.compute_labels_matrix(
            no_background=True, labels2int_map=labels2int_map
        )
...
class BASTeamTDeedEvaluator(TDeedMAPEvaluator):
    def __init__(
        ...
        soccernet_path: Optional[str] = None,
    ):
        self.model = model
        self.dataset = dataset
        self.delta_frames_tolerance = delta_frames_tolerance
        self.soccernet_path = soccernet_path
    ...
    def eval(
        ...
        tmp_solution = os.path.join(tempfile.gettempdir(), f"solution_{time.time()}")
        self.create_solution_file(
            scored_videos=scored_videos,
            zip_output_file_name=tmp_solution,
        )

        map_mine = self.compute_map(
            scored_videos, self.delta_frames_tolerance, len(BASLabel)
        )

        m = {}
        if self.soccernet_path is not None:
    ...
    def create_solution_file(
        ...
        tmp_root_dir = os.path.join(tempfile.gettempdir(), f".sn-{time.time()}")
