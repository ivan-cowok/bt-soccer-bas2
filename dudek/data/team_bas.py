...
def _path_tail_three(video_dir_path: str) -> tuple[str, str, str]:
    """Last three segments (league/season/match). Works on Windows and POSIX paths."""
    parts = Path(os.path.normpath(video_dir_path)).parts
    if len(parts) < 3:
        raise ValueError(
            f"Expected path with at least 3 segments (…/league/season/match), got {video_dir_path!r}"
        )
...
class Team(enum.Enum):
    ...
    @staticmethod
    def from_label_json(raw) -> "Team":
        """Parse team field from SoccerNet JSON (left/right, or home/away)."""
        if raw is None:
...
class ActionLabel(enum.Enum):
    ...
    def to_bas_label(self) -> Optional["BASLabel"]:
        ...
        elif self in [ActionLabel.INDIRECT_FREE_KICK, ActionLabel.DIRECT_FREE_KICK]:
            ...
            with open(labels_path, "r") as f:
                ...
                for label_json in labels_json["annotations"]:
                    if half is None or label_json["gameTime"][0] == str(half):
                        ...
                        label_str = label_json["label"]
                        label_map = {v.value.lower(): v for v in enum_class}
                        label = label_map.get(label_str.lower())
                        if label is None:
                            raise ValueError(f"'{label_str}' is not a valid {enum_class.__name__}")
                        labels.append(
                            Annotation(
                                label=label,
...
class SoccerVideo:
    ...
    def compute_labels_matrix(
        self,
        labels2int_map: Dict[enum.Enum, int],
        no_background: bool = False,
    ):
        """Labels matrix without team dimension — one column per action class."""
        labels_matrix = np.zeros(
            shape=(self.actual_n_frames, len(labels2int_map) + 1), dtype=np.float32
        )
